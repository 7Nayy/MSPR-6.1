...
def reset_password():
    if request.method == 'POST':
        try:
            ...
            expiration = int(time.time()) + 3600  # Expire dans 1 heure
            ...
            # Dans une application réelle, envoyez un email avec le lien de réinitialisation
            # Pour l'instant, nous affichons simplement un message de succès
            app.logger.info(f'Demande de réinitialisation de mot de passe pour: {email}')
            
            return render_template('reset_password.html', 
                                  success="Un email de réinitialisation a été envoyé. Veuillez vérifier votre boîte de réception.")
            
        except ValueError as e:
            app.logger.warning(f'Erreur de réinitialisation de mot de passe: {str(e)}')
